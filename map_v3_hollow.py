"""
Publication-quality study-area map of Kentucky Mesonet stations — hollow-marker variant.
...
- Hollow markers throughout: open circles for retained, open stars for matched
"""
...
C_KY_FILL  = "#ffffff"        # white Kentucky fill
C_NEIGHBOR = "#f2f2f2"        # neighbouring states
C_OCEAN    = "#e8f4fb"        # very pale blue ocean / background
C_KY_EDGE  = "#2c3e50"       # KY border colour
C_RETAIN_E = "#1a6dab"       # retained station edge colour
C_MATCH_E  = "#c07000"       # matched station edge colour
...
for geom in ky_geom:
    ax.add_geometries([geom], DATA_CRS, facecolor="none",
                      edgecolor=C_KY_EDGE, linewidth=1.8, zorder=4)
...
ax.add_feature(
    cfeature.NaturalEarthFeature("physical", "rivers_lake_centerlines", "10m",
                                 facecolor="none", edgecolor="#90c8e0",
                                 linewidth=0.5),
    zorder=5)

# ── station markers ───────────────────────────────────────────────────────────
# Retained stations — hollow blue circles
ax.scatter(retained["Lon"], retained["Lat"],
           s=60, marker="o",
           facecolors="none", edgecolors=C_RETAIN_E, linewidths=1.4,
           transform=DATA_CRS, zorder=6)

# Matched stations — hollow gold stars, on top
ax.scatter(matched["Lon"], matched["Lat"],
           s=150, marker="*",
           facecolors="none", edgecolors=C_MATCH_E, linewidths=1.8,
...
for _, row in matched.iterrows():
    ...
    ax.annotate(
        ...
        fontsize=6.5, fontweight="bold", color="#1a3a5c",
        ha="left", va="bottom",
        bbox=dict(boxstyle="round,pad=0.15", facecolor="#e8f4fb",
...
legend_elements = [
    Line2D([0], [0], marker="o", color="w",
           ...
           markerfacecolor="none", markeredgecolor=C_RETAIN_E,
           markeredgewidth=1.4, markersize=8),
    Line2D([0], [0], marker="*", color="w",
           label=f"Matched stations (n={len(matched)}, ≤2 km)",
           markerfacecolor="none", markeredgecolor=C_MATCH_E,
...
ax_ins.add_feature(cfeature.LAND.with_scale("50m"),
                   facecolor=C_NEIGHBOR, edgecolor="none", zorder=1)
...
# KY highlight: hollow red outline only (no fill)
for geom in ky_geom:
...
fig.savefig("map_v3_hollow.png", dpi=300, **save_kw)
print("Saved → map_v3_hollow.png")
fig.savefig("map_v3_hollow.pdf", **save_kw)
print("Saved → map_v3_hollow.pdf")
