"""
Print-friendly greyscale study-area map of Kentucky Mesonet stations.
Based on generate_map.py — colours replaced with greyscale equivalents.
- Retained stations: solid black circles
- Matched stations (MRHD, SCTV, CRMT, HCKM, WDBY, WLBT): large white stars
- CONUS inset placed just below the legend (same auto-positioning logic)
"""
...
# ── projection / greyscale colours ────────────────────────────────────────────
DATA_CRS  = ccrs.PlateCarree()
PROJ      = ccrs.AlbersEqualArea(central_longitude=-86.0, central_latitude=37.8)

C_KY_FILL  = "#e8e8e8"   # light grey for Kentucky
C_NEIGHBOR = "#c8c8c8"   # medium grey for neighbouring states
C_KY_EDGE  = "#1a1a1a"   # near-black KY border
C_COUNTY   = "#999999"   # county edges
C_STATE    = "#555555"   # state lines
C_RIVER    = "#888888"   # rivers
C_RETAIN   = "#111111"   # retained station circles (near-black)
C_MATCH_EDGE = "#111111" # matched star edge
...
ax.add_feature(
    cfeature.NaturalEarthFeature("cultural", "admin_2_counties", "10m",
                                 facecolor="none", edgecolor=C_COUNTY,
                                 linewidth=0.3),
    zorder=3)
...
for geom in ky_geom:
    ax.add_geometries([geom], DATA_CRS, facecolor="none",
                      edgecolor=C_KY_EDGE, linewidth=1.6, zorder=4)

# 4) State lines
ax.add_feature(
    cfeature.NaturalEarthFeature("cultural",
                                 "admin_1_states_provinces_lines", "50m",
                                 facecolor="none", edgecolor=C_STATE,
                                 linewidth=0.9),
    zorder=4)
...
ax.add_feature(
    cfeature.NaturalEarthFeature("physical", "rivers_lake_centerlines", "10m",
                                 facecolor="none", edgecolor=C_RIVER,
                                 linewidth=0.4),
    zorder=5)

# ── station markers ───────────────────────────────────────────────────────────
# Retained stations: solid black circles with white edge
ax.scatter(retained["Lon"], retained["Lat"],
           s=48, marker="o", color=C_RETAIN,
           edgecolors="white", linewidths=0.6,
           transform=DATA_CRS, zorder=6)

# Matched stations: large white stars with black edge
ax.scatter(matched["Lon"], matched["Lat"],
           s=120, marker="*", color="white",
           edgecolors=C_MATCH_EDGE, linewidths=1.2,
...
legend_elements = [
    ...
    Line2D([0], [0], marker="*", color="w",
           ...
           markerfacecolor="white", markeredgecolor=C_MATCH_EDGE,
...
# ── CONUS inset — placed just below the legend, auto-positioned ───────────────
...
# Greyscale CONUS inset: land #d0d0d0, ocean white, KY #444444 fill + #000000 edge
...
# KY highlight: dark grey fill with black edge
for geom in ky_geom:
...
fig.savefig("map_v1_grayscale.png", dpi=300, **save_kw)
print("Saved → map_v1_grayscale.png")
fig.savefig("map_v1_grayscale.pdf", **save_kw)
print("Saved → map_v1_grayscale.pdf")
